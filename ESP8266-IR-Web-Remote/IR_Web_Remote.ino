...
void handleRoot() {
  ...
  website = website + endDiv;  // ------------------------- Channel Controls --------------------------
  website = website + rowDiv;
  website = website + generateButton("col-xs-3", "tc", "Table colour", "tc");
  ...
  website = website + endDiv;  // ------------------------- Volume Controls --------------------------
  website = website + rowDiv;
  website = website + generateButton("col-xs-12", "OFF", "OFF", "OFF");
  website = website + endDiv;
  website = website + rowDiv;
  website = website + generateButton("col-xs-12", "on", "on", "on");
  website = website + endDiv;
  website = website + rowDiv;
...
void setup(void) {
  ...
  server.on("/sspower", []() {
    Serial.println("default colour crayon");
    irsend.sendNEC(0xF7B04F, 32);
    server.send(200, "text/plain", "default colour crayon");
  });
  server.on("/tc", []() {
    Serial.println("table colour");
    irsend.sendNEC(0xF750AF, 32);
    server.send(200, "text/plain", "table colour");
  });
